...
namespace XOuranos.Configuration
{
    ...
    public class NodeSettings : IDisposable
    {
        ...
        /// </remarks>
        public NodeSettings(Network network = null, string agent = "XOuranos",
            string[] args = null, NetworksSelector networksSelector = null)
        {
            ...
            this.DataDirRoot = this.ConfigReader.GetOrDefault<string>("datadirroot", "XOuranos", this.Logger);
        ...
        public static void PrintHelp(Network network, IFullNodeBuilder fullNodeBuilder)
        {
            ...
            builder.AppendLine($"-datadirroot=<Path>       The path to the root data directory, which holds all node data on the machine. Defaults to 'XOuranos'.");
            builder.AppendLine($"-debug[=<string>]         Set 'Debug' logging level (add -debug=tasks for backround tasks). Specify what to log via e.g. '-debug=XOuranos.Miner,XOuranos.Wallet'.");
