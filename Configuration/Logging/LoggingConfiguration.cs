...
namespace XOuranos.Configuration.Logging
{
    /// <summary>
        ...
        public static ILoggerFactory Create(LogSettings settings)
        {
            return ExtendedLoggerFactory.Create(builder =>
            {
                ...
                builder.AddFilter("Default", LogLevel.Information)
                    ...
                    .AddFilter("Microsoft", LogLevel.Warning)
                    .AddFilter("Microsoft.AspNetCore", LogLevel.Error)
                    .AddFilter<ConsoleLoggerProvider>($"{nameof(XOuranos)}.*", LogLevel.Information)
                    .AddConsole();
    ...
    public static class LoggingConfiguration
    {
        ...
        private static readonly Dictionary<string, string> KeyCategories = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // { "lock", "" },
            // { "mempoolrej", "" },
            { "net", $"{nameof(XOuranos)}.{nameof(Connection)}.*" },
            // { "proxy", "" },
            // { "prune", "" },
            // { "rand", "" },
            // { "reindex", "" },
            ...
            // Short Names
            { "configuration", $"{nameof(XOuranos)}.{nameof(Configuration)}.*" },
            { "fullnode", $"{nameof(XOuranos)}.{nameof(FullNode)}" },
        };
        ...
        private static void AddFilters(LogSettings settings = null, DataFolder dataFolder = null)
        {
            ...
            var defaultRule = new LoggingRule($"{nameof(XOuranos)}.*", settings.LogLevel, mainTarget);
            ...
            if (settings.DebugArgs.Any() && settings.DebugArgs[0] != "1")
            {
                lock (KeyCategories)
                {
                    ...
                    foreach (string key in settings.DebugArgs)
                    {
                        ...
                        {
                            // Allow direct specification - e.g. "-debug=XOuranos.Miner".
                            category = key.Trim();
                        }
            ...
            if (settings != null)
            {
                if (settings.DebugArgs.Any())
                {
                    if (settings.DebugArgs[0] == "1")
                    {
                        ...
                        builder.AddFilter<ConsoleLoggerProvider>($"{nameof(XOuranos)}", Microsoft.Extensions.Logging.LogLevel.Debug);
                    }
                    else
                    {
                        lock (KeyCategories)
