...
using XOuranos.Controllers;
using XOuranos.Features.BlockStore;
using XOuranos.Features.RPC;
using XOuranos.Features.RPC.Exceptions;
using XOuranos.Features.Wallet.Api.Models;
using XOuranos.Features.Wallet.Database;
using XOuranos.Features.Wallet.Exceptions;
...
using System.Net;
using XOuranos.NBitcoin.BIP39;
using XOuranos.Utilities.JsonErrors;
using XOuranos.Utilities.ModelStateErrors;

namespace XOuranos.Features.Wallet.Api.Controllers
{
    public class WalletRPCController : FeatureController
    {
        ...
        public dynamic Create(string walletname, string password)
        {
            var words = "";
            try
            {
                var mm = new Mnemonic(Wordlist.English, WordCount.Twelve);
                Mnemonic mnemonic = this.walletManager.CreateWallet(password, walletname, "", mnemonic: mm
                    , purpose: 84);
                foreach (var wd in mnemonic.Words)
                {
                    if (words.Length > 0)
                    {
                        words += ",";
            ...
            catch (WalletException e)
            {
                ...
                throw new RPCServerException(RPCErrorCode.RPC_INVALID_REQUEST, e.Message);
            }
