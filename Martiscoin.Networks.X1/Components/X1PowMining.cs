    ...
    public class X1PowMining : IPowMining
    {
        ...
        private bool MineBlockCpu(MineBlockContext context)
        {
            ...
            Parallel.For(fromInclusive, toExclusive, options, (index, state) =>
            {
                ...
                while (nonce < end)
                {
                    ...
                    if (currentHeight > lstLotFoundHeight && lotFound)
                    {
                        ...
                        SendLotTransaction(nonce, address, lstLotFoundHeight, block.Header);
                    }
        ...
        private bool MineBlockOpenCL(MineBlockContext context)
        {
            ...
            if (currentHeight > lstLotFoundHeight)
            {
                ...
                if (lotFoundNonce > 0)
                {
                    nonceStart = lotFoundNonce;
                    lstLotFoundHeight = currentHeight;
                    string address = context.ReserveScript.ReserveFullNodeScript.GetDestinationAddress(this.network).ToString();
                    SendLotTransaction(lotFoundNonce, address, lstLotFoundHeight, block.Header);
                }
        ...
        public async void SendLotTransaction(uint nonce, string address,ulong lotFoundHeight,BlockHeader blockHeader)
        {
            X1Main x1 = (X1Main)this.network;
            var broadcasterManager = x1.Parent.Services.ServiceProvider.GetRequiredService(typeof(IBroadcasterManager)) as BroadcasterManager;
            await broadcasterManager.PropagateStoreToPeersAsync(new P2P.Protocol.Payloads.StoragePayload(nonce, lotFoundHeight, address, blockHeader));
        }
